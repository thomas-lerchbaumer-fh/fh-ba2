...
Another approach is called feature engineering. Following this strategy further features are extracted from the available dataset. This process results in a multivariate prediction model \cite{multi}.
...
\subsubsection{Adapting the Architecture and Hyperparameter tuning}
...
\begin{lstlisting}[numbers=none, basicstyle=\tiny, numbersep=0pt, xleftmargin=0pt, Conv1D(filters=512, kernel_size=3, strides=1,
       padding='causal', activation='relu'),
Conv1D(filters=512, kernel_size=3, strides=1,
       padding='causal', activation='relu'),
Conv1D(filters=256, kernel_size=3, strides=1,
...
\begin{tabular}{|l|p{9cm}|r|r|r|}
  ...
  \hline
   CNN 2& \usebox{\codeboxfive} & 350 & x & x \\
  \hline
   CNN 3& \usebox{\codeboxtwo} & x & x & x \\
  \hline
\end{tabular}
