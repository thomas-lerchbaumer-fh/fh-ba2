...
df["isweekend"] = np.where(df.index.day_name().isin(['Saturday', 'Sunday']), 1, 0)
df['week_of_year'] = df.index.isocalendar().week
austrian_holidays = Austria(years=list(range(2017, 2024)))
df['is_holiday'] = df.index.map(lambda x: int(x in austrian_holidays))
df['bookings_roll_mean7'] = df['bookings'].rolling(window=7).mean()
